...
class OpCode(Enum):
    ...
    # ── Bitwise ─────────────────────────────────────────────────────────────
    BAND = 0x18             # Pop b, pop a, push a & b
    BOR = 0x19              # Pop b, pop a, push a | b
    BXOR = 0x1A             # Pop b, pop a, push a ^ b
    BNOT = 0x1B             # Pop a, push ~a
    LSHIFT = 0x1C           # Pop b, pop a, push a << b
    RSHIFT = 0x1D           # Pop b, pop a, push a >> b
    ...
    BUILD_STRING = 0x55     # Pop count items, convert to string, concat, push string

    # ── Async/Coroutine ──────────────────────────────────────────────────────
    MAKE_COROUTINE = 0x80   # Mark function as coroutine (async function)
    AWAIT = 0x81            # Await a coroutine - suspends execution until complete

    # ── Data Structures ─────────────────────────────────────────────────────
    BUILD_LIST = 0x60       # BUILD_LIST count: Pop count items, push list
    BUILD_DICT = 0x61       # BUILD_DICT count: Pop count*2 items, push dict
    BUILD_SET = 0x6F        # BUILD_SET count: Pop count items, push set
    INDEX_GET = 0x62        # Pop idx, pop obj, push obj[idx]
    INDEX_SET = 0x63        # Pop val, pop idx, pop obj, obj[idx] = val
    MAKE_SLICE = 0x6C       # Pop step, stop, start, push slice object
    ...
    LIST_APPEND = 0x6D      # Pop val, list is at stack[-2]. Append val to list.
    UNPACK_SEQUENCE = 0x6E  # UNPACK_SEQUENCE count: Pop obj, unpack into count items, push items
    ...
    WITH_CLEANUP = 0x82     # Call 'बंद_कर' or '__exit__' on context manager object
...
STACK_EFFECT = {
    ...
    OpCode.BUILD_STRING: -999,

    OpCode.BUILD_LIST: -999,  # Variable
    OpCode.BUILD_DICT: -999,  # Variable
    OpCode.BUILD_SET: -999,   # Variable
    OpCode.INDEX_GET: -1,
    OpCode.INDEX_SET: -3,
    OpCode.MAKE_SLICE: -2,
    OpCode.LIST_APPEND: -1,
    OpCode.UNPACK_SEQUENCE: -999,
    OpCode.WITH_CLEANUP: -1,
    OpCode.ATTR_GET: 0,
    OpCode.ATTR_SET: -2,
    ...
    OpCode.MAKE_COROUTINE: 0,  # No stack effect, marks function
    OpCode.AWAIT: 0,           # Pops coroutine, pushes result (conceptually 0 during suspend)
