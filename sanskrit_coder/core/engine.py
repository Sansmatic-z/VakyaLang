...
class SanskritEngine:
    ...
    def __init__(self):
        ...
        self._knowledge_topics = {
            ...
            'nyaya': (
                ...
                "Sanskrit: न्यायदर्शनम् षड्दर्शनेषु अन्यतमम्"
            ),
        }
    ...
    def convert(self, value: str, from_unit: str, to_unit: str) -> str:
        """Convert between a small set of metric units."""
        try:
            numeric_value = self.math.parse_sanskrit_number(value)
        except Exception:
        ...
        if from_key not in self._unit_factors or to_key not in self._unit_factors:
            raise ValueError(f"अज्ञात एककौ: {from_unit} -> {to_unit}")

        base_value = float(numeric_value) * self._unit_factors[from_key]
        converted = base_value / self._unit_factors[to_key]

        if converted.is_integer():
            converted = int(converted)

        return f"{converted} {to_unit}"
    ...
    def cmd_convert(self, args: str) -> str:
        """Convert between supported units."""
        if not args:
            return "उपयोगः: परिवर्तय <value> <from_unit> to <to_unit>"
        ...
        try:
            return self.convert(parts[0], parts[1], parts[3])
        except Exception as e:
            return f"परिवर्तन त्रुटिः {str(e)}"
...
"""
    ...
    def cmd_search(self, args: str) -> str:
        ...
        if topic_key in self._knowledge_topics:
            return self._knowledge_topics[topic_key]
