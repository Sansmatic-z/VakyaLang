...
class SanskritMathEngine:
    ...
    def _parse_compound_number(self, text: str) -> Optional[Union[int, float]]:
        """Parse compound Sanskrit numbers up to 100 systematically."""
        # Base words for units in compounds
        units = {
            'एका': 1, 'एक': 1, 'द्वा': 2, 'द्वि': 2, 'त्रय': 3, 'त्रि': 3, 
            'चतुर्': 4, 'चतुः': 4, 'पञ्च': 5, 'षड्': 6, 'षट': 6, 'सप्त': 7, 
            'अष्टा': 8, 'अष्ट': 8, 'नव': 9, 'एकोन': -1
        }
        
        # Base words for tens
        tens = {
            'दश': 10, 'विंशति': 20, 'विंशती': 20, 'त्रिंशत्': 30, 'चत्वारिंशत्': 40,
            'पञ्चाशत्': 50, 'षष्टि': 60, 'सप्तति': 70, 'सप्तती': 70, 'अशीति': 80, 'नवति': 90, 'नवती': 90,
            'शतम्': 100, 'शत': 100
        }
        
        if text in tens: return tens[text]
        
        # Try systematic decomposition
        # Example: पञ्चविंशति (25) -> पञ्च + विंशति
        for t_word, t_val in tens.items():
            if text.endswith(t_word):
                prefix = text[:-len(t_word)]
                if not prefix: return t_val
                
                # Check for common junctions (e.g., pancha + navati = panchanavati)
                # Remove common filler vowels/consonants at junction
                if prefix.endswith('न'): prefix = prefix[:-1]
                
                if prefix in units:
                    u_val = units[prefix]
                    if u_val == -1: # Handle ekona (one less than)
                        return t_val - 1
                    return t_val + u_val
                
                # Try with vowel variations (e.g., panca vs pancha)
                variations = {'पञ्च': 'पञ्च', 'पञ्चा': 'पञ्च', 'सप्त': 'सप्त', 'सप्ता': 'सप्त', 'अष्ट': 'अष्ट', 'अष्टा': 'अष्ट'}
                if prefix in variations:
                    u_val = units[variations[prefix]]
                    return t_val + u_val

        # Fallback to hardcoded common compounds for stability
        compounds = {
    ...
    def _tokenize_math(self, expr: str) -> List[Tuple[str, Union[str, Decimal, int]]]:
        ...
        while i < len(expr):
            ...
            word = ''
            # Check if this single character is an operator first
            if char in self.operations:
                tokens.append(('OP', self.operations[char]))
                i += 1
                continue
                
            while i < len(expr) and not expr[i].isspace() and expr[i] not in '()':
                if expr[i] in self.operations: # Break on operator
                    break
                word += expr[i]
                i += 1
            ...
            if word:
                if word in self.bhuta_sankhya:
                    ...
                    # Try to parse as compound number before giving up
                    compound = self._parse_compound_number(word)
                    if compound is not None:
                        tokens.append(('NUMBER', compound))
                    else:
                        tokens.append(('UNKNOWN', word))
            else:
                if i < len(expr) and not expr[i].isspace():
                    i += 1
    ...
    def solve_equation(self, equation: str) -> str:
        ...
        simple_solution = self._solve_simple_linear(left, right)
    ...
    def _solve_simple_linear(self, left: str, right: str) -> Optional[str]:
        """Handle basic linear forms like 4*x+8=28 before polynomial fallback."""
        right_value = right.strip()
        try:
            rhs = Decimal(str(eval(right_value, {"__builtins__": {}}, {})))
        except Exception:
            return None

        normalized_left = left.replace(' ', '').replace('*', '')
        match = re.fullmatch(r'([+-]?(?:\d+(?:\.\d+)?)?)x(?:([+-])(\d+(?:\.\d+)?))?', normalized_left)
        if not match:
            return None

        coeff_str, sign, offset_str = match.groups()
        if coeff_str in ('', '+'):
            a = Decimal('1')
        elif coeff_str == '-':
            a = Decimal('-1')
...
"""
    ...
    def lookup_formula(self, query: str) -> str:
        ...
        if normalized_query in self.named_formulas:
            info = self.named_formulas[normalized_query]
            return (
                f"सूत्रम् (Formula): {info['formula']}\n"
                f"संस्कृतम् (Sanskrit): {info['sanskrit']}\n"
                f"नाम (Name): {info['name']}"
            )
