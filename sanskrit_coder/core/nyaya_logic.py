...
class Hetvabhasa:
    ...
    def __init__(self):
        # Known contradictory facts for Badhita detection
        self.fact_base = {
            'अग्नि': {'शीत': False, 'उष्ण': True},  # Fire is hot, not cold
            'जलं': {'शीत': True, 'उष्ण': False},   # Water is cold
            'आकाश': {'मूर्त': False, 'अमूर्त': True} # Ether is formless
        }
    
    def analyze(self, hetu: str, paksha: str, sadhya: str) -> Dict:
        """
        Analyze a reason for fallacies using rules and fact base.
        """
        results = {}
        
        # 1. Check Viruddha (contradictory)
        contradictory_pairs = [
            ('नित्य', 'अनित्य'), ('सत्', 'असत्'), ('सुख', 'दुःख'),
            ('चेतन', 'अचेतन'), ('प्रकाश', 'अन्धकार')
        ]
        
        for p1, p2 in contradictory_pairs:
            if (p1 in sadhya and p2 in hetu) or (p2 in sadhya and p1 in hetu):
                results['विरुद्ध'] = self.FALLACIES['विरुद्ध']

        # 2. Check Badhita (Counter-facted) against fact base
        for subject, traits in self.fact_base.items():
            if subject in paksha or paksha in subject:
                for trait, is_true in traits.items():
                    if (trait in sadhya or sadhya in trait) and not is_true:
                        results['बाधित'] = self.FALLACIES['बाधित']

        # 3. Check Savyabhicara (Too broad) - Basic heuristic
        broad_terms = ['ज्ञेयत्वात्', 'प्रमेयत्वात्', 'प्रमेय', 'knowable', 'object']
        if any(term in hetu for term in broad_terms):
            results['सव्यभिचार'] = self.FALLACIES['सव्यभिचार']
